...
class Motion {
	...
	public String toString() {
		return "MOTION of " + getAircraft().toString() + " from " + getFirstPosition().toString()
	...
	public Vector3d findIntersection(final Motion other) {
		...
		final float a = (vx2 - vx1) * (vx2 - vx1) + (vy2 - vy1) * (vy2 - vy1) + (vz2 - vz1)
				* (vz2 - vz1);
		...
		if (a != 0.0f) {
			...
			// b = 2 * <I1-I2, V1-V2>
			float b = 2.0f * (i2.x * vx2 - i2.x * vx1 - i1.x * vx2 + i1.x * vx1 + i2.y * vy2 - i2.y
					* vy1 - i1.y * vy2 + i1.y * vy1 + i2.z * vz2 - i2.z * vz1 - i1.z * vz2 + i1.z
					* vz1);

			// c = -r^2 + (I2 - I1)^T * (I2 - I1)
			final float c = -r * r + (i2.x - i1.x) * (i2.x - i1.x) + (i2.y - i1.y) * (i2.y - i1.y)
					+ (i2.z - i1.z) * (i2.z - i1.z);
			...
			if (v1 <= v2
					&& (v1 <= 1.0f && 1.0f <= v2 || v1 <= 0.0f && 0.0f <= v2 || 0.0f <= v1
							&& v2 <= 1.0f)) {
				...
				if (z1col > Constants.MIN_Z && z1col <= Constants.MAX_Z && x1col >= Constants.MIN_X
						&& x1col <= Constants.MAX_X && y1col >= Constants.MIN_Y
						&& y1col <= Constants.MAX_Y)
			...
			float dist = (i2.x - i1.x) * (i2.x - i1.x) + (i2.y - i1.y) * (i2.y - i1.y)
					+ (i2.z - i1.z) * (i2.z - i1.z);
