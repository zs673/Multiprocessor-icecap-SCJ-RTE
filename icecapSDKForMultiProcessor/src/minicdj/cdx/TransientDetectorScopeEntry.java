...
public class TransientDetectorScopeEntry implements Runnable {
	...
	public void run() {
		...
		if (Constants.SYNCHRONOUS_DETECTOR || Constants.DEBUG_DETECTOR) {
			devices.Console.println("CD detected  " + numberOfCollisions + " collisions.");
			/*
	...
	public void dumpFrame(String debugPrefix) {
		...
		for (int i = 0; i < currentFrame.planeCnt; i++) {
			...
			devices.Console.println(prefix + new String(currentFrame.callsigns, offset, cslen)
					+ " " + currentFrame.positions[3 * i] + " " + currentFrame.positions[3 * i + 1]
	...
	public List createMotions() {
		...
		for (int i = 0, pos = 0; i < currentFrame.planeCnt; i++) {
			...
			if (old_pos == null) {
				...
				// the state table.
				state.put(mkCallsignInPersistentScope(craft.getCallsign()), new_pos.x, new_pos.y,
						new_pos.z);

				final Motion m = new Motion(craft, new_pos);
				if (minicdj.cdx.Constants.DEBUG_DETECTOR
						|| minicdj.cdx.Constants.SYNCHRONOUS_DETECTOR) {
					devices.Console.println("createMotions: old position is null, adding motion: "
							+ m);
				}
