...
public class FullStackAnanlyser implements StackAnalyser {
			...
			public void run() {
				...
				for (byte index = 0; index < stacks.length; index++) {
					if (stacks[index] != null) {
						...
						devices.Console.println("stack " + index + "[" + best_start_of_unused_area
								+ ", " + best_end_of_unused_area + "][" + stacks[index].length
								+ "]");
					}
				}
			}
	...
	private void updateBest(int start_of_unused_area, int end_of_unused_area) {
		if (end_of_unused_area - start_of_unused_area > best_end_of_unused_area
				- best_start_of_unused_area) {
