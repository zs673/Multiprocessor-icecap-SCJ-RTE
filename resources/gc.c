...
Object* gc_allocateObject(uint32 dobjectSize, uint32 pobjectsize) {
    if (pobjectsize == 0) {
       ...
       /* top = HEAP_REF(currentMemoryArea, VMMemory* )->free;*/
        ...
        /*obj = gc_allocatepdObject(dobjectSize + sizeof(Object), (unsigned char*) (pointer) HEAP_REF(currentMemoryArea, VMMemory* )->base,
        &top, HEAP_REF(currentMemoryArea, VMMemory* )->size, 1);*/
...
#if defined(VM_MEMORY_UPDATEMAXUSED_USED)
        vm_Memory_updateMaxUsed(0, (int32)(pointer)current);
#endif
        unlock_memory();
        return obj;
    }
