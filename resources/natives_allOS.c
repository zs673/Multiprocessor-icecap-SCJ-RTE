...
#if defined(N_JAVA_LANG_REFLECT_METHOD_INVOKE) || defined(N_JAVA_LANG_CLASS_GETCONSTRUCTOR) || defined(N_JAVA_SECURITY_ACCESSCONTROLLER_DOPRIVILEGED) || defined(ENABLE_DEBUG) || defined(N_JAVA_LANG_CLASS_NEWINSTANCE) || defined(N_JAVA_LANG_CLASS_GETMETHOD)
...
extern unsigned char* createArrayFromElementSize(unsigned short classIndex, unsigned char elementSize, uint16 count FLASHARG(uint8 flash));
extern int16 initializeException(int32* sp, int16 exceptionClass, int16 exceptionInitMethod);
...
extern unsigned char getField(unsigned char *data, unsigned char size, int32* sp);
...
static char* getCString(unsigned char* strObj) {
    ...
    getField(HEAP_REF(strObj + offset, unsigned char*), 32, (int32*) &string_valuep);
...
#else
    string_count = *((uint16*) (pointer) HEAP_REF(string_valuep, unsigned char*) + 1);
#endif
...
static void arraycopy(unsigned char* src, unsigned short srcPos, unsigned char* dst, unsigned short dstPos, unsigned short length) {
...
#else
    src = HEAP_REF(src, unsigned char*) + sizeof(Object) + 2 + (srcPos * elementSize);
    dst = HEAP_REF(dst, unsigned char*) + sizeof(Object) + 2 + (dstPos * elementSize);
...
Object* createStringObject(int32 size, const char* data, int32* sp) {
    ...
    pushDefaultArea();
    charArrayObject = createArray((unsigned short) _C_var, (uint16) size FLASHARG(0));

    if (charArrayObject != 0) {

        for (count = 0; count < size; count++) {
            *((int32 *) (HEAP_REF(charArrayObject, unsigned char *) + sizeof(Object) + 2) + count) = pgm_read_byte(data + count);
        }
...
Object* getClass(unsigned short classIndex) {
    Object *class = head;
    while (class != 0) {
        if (*(unsigned short *) ((unsigned char*) HEAP_REF(class, Object*) + sizeof(Object)) == classIndex) {
            return class;
        } else {
            class = *(Object **) ((unsigned char*) HEAP_REF(class, Object*) - sizeof(Object*));
        }
    }
    {
        unsigned short dobjectSize = pgm_read_word(&classes[JAVA_LANG_CLASS_var].dobjectSize) >> 3;
        unsigned short pobjectSize = pgm_read_word(&classes[JAVA_LANG_CLASS_var].pobjectSize) >> 3;
        ...
        if (class != 0) {
            ...
            *(unsigned short *) ((unsigned char*) HEAP_REF(class, Object*) + sizeof(Object)) = classIndex;
            *(Object **) ((unsigned char*) HEAP_REF(class, Object*) - sizeof(Object*)) = head;
            head = class;
        } else {
            return 0;
        }
...
#if defined(N_JAVA_SECURITY_ACCESSCONTROLLER_DOPRIVILEGED) || defined(N_JAVA_LANG_CLASS_NEWINSTANCE)
...
int16 n_java_security_AccessController_doPrivileged(int32 *sp) {
    if (JAVA_LANG_BOOLEAN_var != -1) {
        return newInstance(sp, JAVA_LANG_BOOLEAN_var);
    } else {
        sp[0] = 0;
    }
    return -1;
...
/*A nanosleep apporach*/
/*void pthread_sleep(long period) {
	struct timespec itval;

	itval.tv_sec = period / 1000000000;
	itval.tv_nsec = period % 1000000000;

	int ret = clock_nanosleep(CLOCK_MONOTONIC, 0, &itval, NULL);
	if( ret !=0 ){
		 printf("clock_nanosleep errno: %d. ret: %d.\n",errno, ret);
	}
}*/
...
{
    sp[1] = sp[0];
    /*printf("n_java_lang_Thread_init_default.\n");*/
...
int16 initializeStringConstant(const ConstantInfo* constant, int32* sp) {
    ...
    if (stringConstants[stringID] == 0) {
        ...
        stringObject = createStringObject(length, (const char *) pgm_read_pointer(&constant->data, const void**), sp);
...
int16 invokeClassInitializers(int32* sp) {
    ...
    while (current < NUMBEROFCLASSINITIALIZERS_var) {
        int16 excep;
        excep = invokeClassInitializer(pgm_read_word(classInitializerSequence + current), sp);
        if (excep > -1) {
            return excep;
        }
        current++;
...
#endif
    initGC();
    printf("n_devices_System_resetMemory \n");
