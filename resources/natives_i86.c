...
uint32 m_base;
uint32 m_new_free;
uint32 heapArea_free;
uint32 heapArea_free_copy;

VMMemory* m = pthread_getspecific(key);
m_free = HEAP_REF(m, VMMemory* )->free;
m_base = HEAP_REF(m, VMMemory* )->base;


if(m_base != HEAP_REF(heapArea, VMMemory* )->base){
    lock_memory();
    heapArea_free = m_free + m_base - HEAP_REF(heapArea, VMMemory* )->base ;
    heapArea_free_copy = HEAP_REF(heapArea, VMMemory* )->free;
    HEAP_REF(heapArea, VMMemory* )->free = heapArea_free;
...
/*if(m_base != HEAP_REF(heapArea, VMMemory* )->base ){
    heapArea_free = HEAP_REF(heapArea, VMMemory* )->free;
    HEAP_REF(heapArea, VMMemory* )->free = heapArea_free_copy;

    m_new_free = heapArea_free - m_base + HEAP_REF(heapArea, VMMemory* )->base ;
    HEAP_REF(m, VMMemory* )->free = m_new_free;

    pthread_setspecific(key, m);
    unlock_memory();
}*/
...
if(ret != 0){
printf("pthread_mutexattr_setprotocol error %d\n", ret);
}*/
    ...
    unsigned char* obj = HEAP_REF(monitor->mutex_f, unsigned char*);
    pthread_mutex_t *mutex_1 = (pthread_mutex_t *) HEAP_REF((pthread_mutex_t *) (pointer) monitor->mutex_f, pthread_mutex_t *);
...
int16 n_thread_JavaLangThreadScheduler_JavaLangThreadMonitor_acquireMutex(int32 *sp) {
   ...
   /* pthread_mutex_t *mutex = (pthread_mutex_t *) (pointer) monitor->mutex_f;
    ...
    int ret = pthread_mutex_lock((pthread_mutex_t *) HEAP_REF(mutex, pthread_mutex_t *));*/
    unsigned char* obj = HEAP_REF(monitor->mutex_f, unsigned char*);

    pthread_mutex_t *mutex = (pthread_mutex_t *) HEAP_REF((pthread_mutex_t *) (pointer) monitor->mutex_f, pthread_mutex_t *);;

    int ret = pthread_mutex_lock(mutex);

    if( ret != 0){
    	printf("acquireMutex errno: %d.\n",ret);
        return initializeException(sp, JAVA_LANG_NULLPOINTEREXCEPTION_var, JAVA_LANG_NULLPOINTEREXCEPTION_INIT__var);
    }
...
int16 n_thread_JavaLangThreadScheduler_JavaLangThreadMonitor_releaseMutex(int32 *sp) {
    ...
    if( pthread_mutex_unlock((pthread_mutex_t *) HEAP_REF(mutex, pthread_mutex_t *)) != 0){
    	printf("releaseMutex errno: %d.\n",errno);
        return initializeException(sp, JAVA_LANG_NULLPOINTEREXCEPTION_var, JAVA_LANG_NULLPOINTEREXCEPTION_INIT__var);
    }
    return -1;
}
...
int16 n_thread_JavaLangThreadScheduler_waitOnCondition(int32 *sp)
{
	...
	/*printf("%s\n", "wait up");*/
    if (ret != 0) {
		printf("waitOnCondition errno: %d.",ret);
		return;
	}
...
void init_memory_lock(void) {
    pthread_mutexattr_t mattr; 
    pthread_mutexattr_init(&mattr);
    int ret = pthread_mutexattr_settype(&mattr, PTHREAD_MUTEX_RECURSIVE);
    if(ret != 0){
        printf("pthread_mutexattr_settype error %d\n", ret);
    }

    pthread_mutex_init(&mem_mutex, &mattr);
